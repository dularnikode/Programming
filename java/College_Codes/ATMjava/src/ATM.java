...
public class ATM {
	...
	public static void main(String[] args) {
		...
		User aUser = theBank.addUser("Keyur", "Jaiswal", "1234");
		User bUser = theBank.addUser("Dullar","Nikode","2345");
		
		
		// add a checking account for our user
		Account newAccount = new Account("Checking", aUser, theBank);
		Account bAccount = new Account("Checking", bUser, theBank);
		aUser.addAccount(newAccount);
		bUser.addAccount(newAccount);
		theBank.addAccount(newAccount);
		theBank.addAccount(bAccount);
	...
	public static User mainMenuPrompt(Bank theBank, Scanner sc) {
		...
		} while(authUser == null); 	// run until we lon in sucessfuly	
		return authUser;
		
	}
	
	//Print the ATM  menu for user actions.
	...
	*/
	...
	//Process transferring funds from one account to another.
		public static void transferFunds(User theUser, Scanner sc) {
			...
			// get account to transfer from
			do {
				System.out.printf("Enter the USER of the account to transfer: ");
				toAcct = sc.nextInt()-1;
				fromAcct =theUser;
